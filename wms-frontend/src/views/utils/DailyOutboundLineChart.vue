<template>
  <div class="chart-container">
    <h3 class="chart-title">近7天出库趋势</h3>
...
import { getDailyOutboundStatistic } from '../../api/statistics'

export default {
  name: 'DailyOutboundLineChart',
  data() {
    return {
      data: [],
      loading: false,
  ...
  methods: {
    async fetchData() {
      ...
      this.error = false
      try {
        const response = await getDailyOutboundStatistic()
    ...
    handleResize() {
      if (this.chart && !this.chart.isDisposed()) {
        try {
          ...
          console.warn('DailyOutboundLineChart - resize错误:', e)
        }
      }
    }
  }
