...
import { getHotGoodsStatistics } from '../../api/statistics'
...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      this.error = false
      try {
        const response = await getHotGoodsStatistics(10)
        console.log('HotGoods - API响应:', response)
        if (response.code === 200) {
          this.data = response.data
          console.log('HotGoods - 数据:', this.data)
          this.loading = false
          await this.$nextTick()
          this.renderChart()
        } else {
    ...
    renderChart() {
      if (!this.$refs.chart) {
        console.warn('HotGoods - chart ref不存在')
        return
      }
    ...
    handleResize() {
      if (this.chart && !this.chart.isDisposed()) {
        try {
          ...
          console.warn('HotGoods - resize错误:', e)
        }
      }
    }
  }
